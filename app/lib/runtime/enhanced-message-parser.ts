...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _commandPatternMap = new Map<string, RegExp>([
    ...
    ['filesystem', /^(cat|chmod|cp|echo|hostname|kill|ln|ls|mkdir|mv|ps|pwd|rm|rmdir|xxd)\s*/],
    ['filesystem-extended', /^(alias|cd|clear|env|false|getconf|head|sort|tail|touch|true|uptime|which)\s*/],
