...
export class FilesStore {
  #sandboxProxy: Promise<{ workdir: string; sandboxId: string; projectId: string }>;
  ...
  async saveFile(filePath: string, content: string) {
    ...
    try {
      ...
      await writeFileToSandbox(sandbox.projectId, relativePath, content);
  ...
  async #init() {
    ...
    // File watcher replaced by sandbox backend — polling to be implemented in a future task
  ...
  async createFile(filePath: string, content: string | Uint8Array = '') {
    ...
    try {
      ...
      if (isBinary) {
        await writeFileToSandbox(sandbox.projectId, relativePath, Buffer.from(content).toString('base64'));
        ...
        await writeFileToSandbox(sandbox.projectId, relativePath, contentToWrite);
  ...
  async createFolder(folderPath: string) {
    try {
      // Folders are created implicitly by the sandbox backend when files are written.
      // We only track the folder locally in the file map.
  ...
  async deleteFile(filePath: string) {
    ...
    try {
      ...
      await deleteFileFromSandbox(sandbox.projectId, relativePath);
